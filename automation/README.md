...
The worker uses a dedicated Chrome profile and imports the episode `STEP6.json` directly, so no Windows file-picker interaction is required. The P&P Studio runs inside an out-of-process `about:srcdoc` iframe; the worker attaches to that iframe explicitly and accepts only a file input whose `accept` contract is `.json` or `application/json`. It must never fall back to the outer Google Flow `image/*` uploader. Read progress at `GET /v1/flow/status`. The local `Access_Key.txt` is read only at runtime and is ignored by Git. Google sign-in is a one-time owner action in the dedicated profile; CAPTCHA, account mismatch, credit purchase and YouTube upload remain stop conditions.
...
AICompanyOS may explicitly select either configured upload account and destination (`kid` / CubeLoom Adventures or `history` / War & History Vault). Each channel has a separate local OAuth profile. The Publishing page shows whether each profile is connected, can match the legacy credential with **ตรวจบัญชี YouTube**, and opens Google's account chooser with **เชื่อมบัญชีที่เลือก**. OAuth profiles remain under the Windows user's local application data and are never returned to the UI.

The bridge accepts the channel group together with the exact configured channel ID and name, rejects arbitrary or mismatched identities before OAuth, applies the selected channel's category and made-for-kids policy, and compares the live OAuth channel ID again before any upload bytes are sent. Upload is disabled until the selected channel profile is identity-verified. One upload batch must target only one YouTube channel.
